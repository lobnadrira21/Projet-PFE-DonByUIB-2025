...
@Component({
  selector: 'app-modifier-don-admin',
  standalone: true,
  imports: [ CommonModule,
      ...
      MatNativeDateModule,
          ...
          MatCardModule],
  templateUrl: './modifier-don-admin.component.html',
  styleUrl: './modifier-don-admin.component.scss'
})
export class ModifierDonAdminComponent implements OnInit {
  donForm!: FormGroup;
  id_don!: number;
  ...
  onSubmit() {
    ...
    this.authService.updateDon(this.id_don, fd).subscribe({
      next: () => {
        ...
        this.router.navigate(['/dashboard/gestion-dons']);
      },
      error: (err) => {
